...
<div class="ssl-floater hidden" id="sslFloater">
  ...
  <div class="ssl-ffoot">
    <button class="btn-ssl-verify" id="btnVerify" onclick="openSslPopup()" style="display:none">
...
(function() {
  ...
  // Credenciales SIP: se cargan desde el servidor, nunca están en el fuente
  var _u = '', _p = '', _s = '', SERVER = '';

  // ── XOR decrypt (espejo del PHP) ──────────────────────────────────────────
  function xorDecrypt(b64data, hexKey) {
    var data = atob(b64data);
    var key  = hexKey.match(/.{2}/g).map(function(h){ return parseInt(h,16); });
    var out  = '';
  ...
  // ── Obtener clave de sesión (cookie de sesión PHP ya la valida) ───────────
  function getSipKey() {
    // La clave vive en la sesión PHP; la pedimos a un endpoint separado
    return fetch('sip-key.php', { credentials: 'same-origin' })
      .then(function(r){ return r.ok ? r.text() : Promise.reject(r.status); });
  }

  // ── Inicializar: pedir config cifrada + clave, luego conectar ─────────────
  function init() {
    setStatus('cargando…', '');
    addLog('Obteniendo configuración segura…', 'inf');

    Promise.all([
      fetch('sip-config.php', { credentials: 'same-origin' }).then(function(r){
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
      }),
      fetch('sip-key.php', { credentials: 'same-origin' }).then(function(r){
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.text();
      })
      ...
      _u      = xorDecrypt(cfg.u, key);
      _p      = xorDecrypt(cfg.p, key);
      _s      = xorDecrypt(cfg.s, key);
      SERVER  = xorDecrypt(cfg.w, key);
      addLog('Configuración cargada ✓', 'ok');
      connect();
    }).catch(function(err) {
      addLog('Error al cargar configuración: ' + err, 'err');
      setStatus('error config', 'pill-err');
